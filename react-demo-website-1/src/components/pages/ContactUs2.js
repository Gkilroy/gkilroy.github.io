//-- ContactUs2.js ---

import { useEffect, useState, useRef } from "react";

//-- form validation
//
//-- controlled - using react state
//-- uncontrolled - form maintains state on submit ie: Document.form[0]
//-- refactor: HTML5 validation
//

const ContactUs2 = (props) => {

    const [msg, setMsg] = useState("");

    const [txtname, setTxtName] = useState("");
    const [nameerror, setNameError] = useState("");   

    const [txtemail, setTxtEmail] = useState("");
    const [emailerror, setEmailError] = useState("");   
    
    useEffect(()=>{

        //-- avoid state/data update and infinate loops 

        //class ComponentDisMound ie: OnLoad()

        return ()=>{
            //component unMound = ie: UnLoad()
        }

    },[]);

    const formSubmit = (e) =>{

        e.preventDefault();
        
        let _msg = "** formSubmit2";

        try {

            _msg = `name: ${txtname}, email: ${txtemail}`;

            setMsg((prevval)=>_msg);
            console.log(_msg)
            //alert(_msg)

            setNameError((preval)=>"");
            setEmailError((preval)=>"");

            if(txtname === null | txtname === undefined | txtname === "" | txtname.trim().length === 0)
            {
                const _msgerr = "* please enter a valid name";
                setNameError((preval)=>_msgerr)
            }

            if(txtemail === null || txtemail === undefined || txtemail === "" || txtemail.trim().length === 0)
            {
                const _msgerr = "* please enter a valid eamil";
                setEmailError((preval)=>_msgerr)
            }

        } catch (error) {
            
            const _err = `${_msg}::err - ${error}`;

            console.log(_err);
            setMsg((prevval)=>_err);
        }

    }

    const validateEmail = (e) => {

        e.preventDefault();
        
        let _msg = "";

        try {

            _msg = `name: ${txtemail}`;

            setMsg((prevval)=>_msg);
            console.log(_msg)

            setTxtEmail((preval)=>e.target.value);

            setEmailError((preval)=>"");

            //if(txtemail === null | txtemail === undefined | txtemail === "" | txtemail.trim().length === 0)
            if(txtemail === null || txtemail === undefined || txtemail === "" || txtemail.trim().length === 0)
            {
                const _msgerr = "* please enter a valid eamil";
                setEmailError((preval)=>_msgerr)
            }

        } catch (error) {
            
            const _err = `validateEmail::err - ${error}`;

            console.log(_err);
            setMsg((prevval)=>_err);
        }

    } 

    return(
        <>
            <h4>ContactUs Page - 2.0.1</h4>
            <p>* controlled form validation example - save form data in usestate</p>
            <span>{msg}</span><span></span>
            <div> 
                <form onSubmit={formSubmit}>
                    <div>
                        <label>* name: </label>
                        <input type="text" placeholder="*name required" maxlength="25"
                        onChange={(e)=> setTxtName(e.target.value)}/>
                        <label>{nameerror}</label>
                    </div>
                    <div>
                        <label>* email: </label>
                        <input type="text" placeholder="*email required" maxlength="50"
                        onChange={(e)=>validateEmail(e)}/>
                        <label>{emailerror}</label>
                    </div>
                    <p></p>
                    <div>
                        <button type="submit">Submit</button>
                    </div>
                </form>
            </div>
        </>
    )
}

export default ContactUs2;